...
export default function Sidebar({ open, setOpen }: Props) {
  ...
  const stokLinks: NavLink[] = [
    ...
    {
      label: "Tambah Produk",
      href: "/dashboard/stokgudang/tambahproduk",
      icon: PlusCircle,
    },
    {
      label: "Tambah Stok",
      href: "/dashboard/stokgudang/tambahstok",
      icon: ArrowDownCircle,
    },
  ];

  const suratLinks: NavLink[] = [
    {
      label: "Surat Jalan",
      href: "/dashboard/suratjalan",
      icon: FileText,
    },
    {
      label: "Buat Surat Jalan",
      href: "/dashboard/suratjalan/formsuratjalan",
      icon: Truck,
    },
  ];
  ...
  const stokActive = stokLinks.some((l) => pathname.startsWith(l.href));
  const suratActive = suratLinks.some((l) => pathname.startsWith(l.href));
  ...
  return (
    <>
      ...
      >
        ...
        {/* MENU */}
        <nav className="flex h-[calc(100%-4rem)] flex-col gap-5 overflow-y-auto px-4 py-4">
          {/* MAIN MENU */}
          <div>
          ...
          {/* STOK GUDANG ACCORDION */}
          <div className="space-y-2">
            <button
              type="button"
              onClick={() => setStokOpen((v) => !v)}
              aria-expanded={stokOpen}
              className={`flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-left text-sm
              border transition-colors
              ${stokActive
                  ? "border-indigo-500/70 bg-slate-900 text-indigo-100"
              ...
              <span className="flex items-center gap-3">
                ...
                <span className="flex flex-col">
                  ...
                  <span className="text-[11px] text-slate-400">
                    Stok, produk, dan pergerakan
                  </span>
                </span>
              </span>
              <ChevronDown
                size={16}
                className={`text-slate-400 transition-transform ${stokOpen ? "rotate-180" : ""
                  }`}
              />
            </button>
            ...
            {stokOpen && (
              <div className="space-y-1 rounded-lg border border-slate-800 bg-slate-950/90 px-3 py-2">
                {stokLinks.map(renderSubLink)}
              </div>
            )}
          </div>

          {/* SURAT JALAN ACCORDION */}
          <div className="space-y-2">
            <button
              type="button"
              onClick={() => setSuratOpen((v) => !v)}
              aria-expanded={suratOpen}
              className={`flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-left text-sm
              border transition-colors
              ${suratActive
            ...
            >
              <span className="flex items-center gap-3">
                ...
                <span className="flex flex-col">
                  ...
                  <span className="text-[11px] text-slate-400">
                    Daftar &amp; pembuatan surat jalan
                  </span>
                </span>
              </span>
              <ChevronDown
                size={16}
                className={`text-slate-400 transition-transform ${suratOpen ? "rotate-180" : ""
                  }`}
              />
            </button>
